...
for results_dir in results_dirs:
    ...
    with open(history_path + "/train_hist", "rb") as file:
        history = pickle.load(file)
    plt.plot(history['acc'], color="C0", label=(results_dir + "_train"), linestyle="-")
    plt.plot(history['val_acc'], color="C1", label=(results_dir + "_valid"), linestyle=":")
    plt.title("Results")
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
    # plt.legend()
    plt.grid()
    # plt.ylim(0, 1)
plt.savefig("./images/" + "matome_from_dec_26-jan_16")
plt.show()