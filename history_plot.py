...
with open(history_path + "/train_hist", "rb") as file:
    history = pickle.load(file)
plt.plot(history['acc'], linestyle="-", color="C1")
plt.plot(history['val_acc'], linestyle="-.", color="C1")
max_val_index = np.argmax(history["acc"])
max_val = history["acc"][max_val_index]
plt.scatter(max_val_index, history["acc"][max_val_index], marker="x", color="C1", label=("実験1最大学習正解率: " + str(max_val.round(3))))
max_val_index = np.argmax(history["val_acc"])
max_val = history["val_acc"][max_val_index]
plt.scatter(max_val_index, history["val_acc"][max_val_index], marker="o", color="C1", label=("実験1最大検証正解率: " + str(max_val.round(3))))

history_path = "./results/" + "keras_2020_01_23_11_56"
with open(history_path + "/train_hist", "rb") as file:
    history = pickle.load(file)
plt.plot(history['acc'], linestyle="-", color="C0")
plt.plot(history['val_acc'], linestyle="-.", color="C0")
max_val_index = np.argmax(history["acc"])
max_val = history["acc"][max_val_index]
plt.scatter(max_val_index, history["acc"][max_val_index], marker="x", color="C0", label=("実験2最大学習正解率: " + str(max_val.round(3))))
max_val_index = np.argmax(history["val_acc"])
max_val = history["val_acc"][max_val_index]
plt.scatter(max_val_index, history["val_acc"][max_val_index], marker="o", color="C0", label=("実験2最大検証正解率: " + str(max_val.round(3))))
