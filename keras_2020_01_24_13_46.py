...
def model(input_shape):
    ...
    X = Conv3D(filters=32, kernel_size=kernel_size, padding="same", strides=(1, 1, 1), activation="elu")(X_input)
    X = Conv3D(filters=64, kernel_size=kernel_size, padding="same", strides=(1, 1, 1), activation="elu")(X)
    X = Conv3D(filters=128, kernel_size=kernel_size, padding="same", strides=(1, 1, 1), activation="elu")(X)
    
    # conv_3 = Conv3D(filters=filters, kernel_size=kernel_size, padding="same", strides=(1, 1, 1), activation="elu")(X_input)
    shape = X.get_shape().as_list()
    
    X = Reshape([shape[1], shape[2]*shape[3]*shape[4]])(X)
    X = Dense(dense_1, activation="elu")(X)
    ...
    X = Bidirectional(LSTM(units=recurrent_units, return_sequences=True, unit_forget_bias=True, dropout=dropout_prob, recurrent_dropout=recurrent_dropout_prob))(X)
    # X = Bidirectional(LSTM(units=recurrent_units, return_sequences=True, unit_forget_bias=True, dropout=dropout_prob, recurrent_dropout=recurrent_dropout_prob))(X)
    X = Bidirectional(LSTM(units=recurrent_units, return_sequences=False, unit_forget_bias=True, dropout=dropout_prob, recurrent_dropout=recurrent_dropout_prob))(X)
    
    shape_rnn_out = X.get_shape().as_list()
    X = Dense(dense_2, activation="elu")(X)
    X = Dropout(dropout_prob)(X)
    X = Dense(n_labels)(X)
    X = Softmax()(X)
